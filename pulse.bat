...
:: ARGUMENTS `pulse.bat [VGM PATH] [CHANNEL ID (1 = PULSE #1, 2 = PULSE #2)] [COMMAND BEGINNING INDEX] [OPTIONS]` AND OTHERS ::
SET "BN_VGMPATH=%~1"
...
SET /A "BN_AOUT_FILTER=(%~4 >> 0) & 1"

:: GLOBAL CONSTANTS ::
SET /A "BN_GC_CLOCK_BASE=(315000000 / 88) / 4"
SET /A "BN_GC_CLOCK_120HZ=BN_GC_CLOCK_BASE / 120"
SET /A "BN_GC_CLOCK_OPT_96HZ=(BN_GC_CLOCK_BASE / 96) - BN_GC_CLOCK_120HZ"
...
:A
SET "BN_EMU_RENDERSAMPLES=0"
FOR /F "TOKENS=1,2,3 USEBACKQ" %%A IN (`RWIB.EXE "!BN_VGMPATH!" READ DEC !BN_POINTER! 3`) DO ( 
	SET /A "BN_POINTER+=1"
	...
	IF %%A EQU 180 (
		...
		IF %%B EQU !BN_REG0! (
			...
			SET "BN_CHSTAT_PULSE_ENV_VOL=15"
			SET /A "BN_CHSTAT_PULSE_ENV_COUNTER=BN_CHREG_PULSE_VOL * BN_GC_CLOCK_120HZ + (BN_APUREG_FCMODE * BN_GC_CLOCK_OPT_96HZ)"
			...
			SET /A "BN_CHSTAT_PULSE_LEN_COUNTER=!BN_TABLE_LEN:~%BN_CHSTAT_PULSE_LEN_COUNTER%,4! * BN_GC_CLOCK_120HZ + (BN_APUREG_FCMODE * BN_GC_CLOCK_OPT_96HZ)"
			SET "BN_CHSTAT_PULSE_PHASE=0"
...
FOR /L %%A IN (1, 1, %~1) DO (
	FOR /L %%B IN (1, 1, 20) DO (
		...
		IF !BN_CHSTAT_PULSE_ENV_COUNTER! EQU 0 (
			SET /A "BN_CHSTAT_PULSE_ENV_COUNTER=BN_CHREG_PULSE_VOL * BN_GC_CLOCK_120HZ + (BN_APUREG_FCMODE * BN_GC_CLOCK_OPT_96HZ)"
			SET /A "BN_CHSTAT_PULSE_ENV_VOL-=1"
			
		) ELSE (
			SET /A "BN_CHSTAT_PULSE_ENV_COUNTER-=1"
			
		)
		
		IF !BN_CHSTAT_PULSE_ENV_VOL! EQU -1 SET /A "BN_CHSTAT_PULSE_ENV_VOL=BN_CHREG_PULSE_LENHALT * 15"
	...
	SET /A "BN_CHSTAT_PULSE_LEVEL=((BN_CHREG_PULSE_CV * BN_CHREG_PULSE_VOL) + (1 - BN_CHREG_PULSE_CV * BN_CHSTAT_PULSE_ENV_VOL)) * (1 - ^!BN_CHSTAT_PULSE_LEN_COUNTER) * BN_APUREG_CHENABLE"
	...
	RWIB.EXE TEST.RAW WRITE DEC !BN_AOUT_OUTPUT!
)
GOTO :EOF