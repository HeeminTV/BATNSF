...
:: GZ DETECTION ::
FOR /F "TOKENS=* USEBACKQ" %%A IN (`RWIB.EXE "!BN_VGMPATH!" READ DEC 0 2`) DO IF "%%A"=="31 139" (
...
FOR /F "TOKENS=* USEBACKQ" %%A IN (`RWIB.EXE "!BN_VGMPATH!" READ DEC 0 4`) DO IF NOT "%%A"=="86 103 109 32" (
	ECHO ======== !BN_SWNAME! Version !BN_VER! ========
	ECHO     Copyright 2025 Heemin
...
FOR /F "TOKENS=1,2 USEBACKQ" %%A IN (`RWIB.EXE "!BN_VGMPATH!" READ DEC 8 2`) DO (
...
:: - NES DETECTION
FOR /F "TOKENS=1 USEBACKQ" %%A IN (`RWIB.EXE "!BN_VGMPATH!" READ DEC 132 1`) DO IF "%%A"=="0" (
	ECHO ======== !BN_SWNAME! Version !BN_VER! ========
	ECHO     Copyright 2025 Heemin
...
:: OFFSET CACULATION ::
FOR /F "TOKENS=1,2 USEBACKQ" %%A IN (`RWIB.EXE "!BN_VGMPATH!" READ DEC 52 2`) DO SET /A "BN_POINTER=((%%B << 8) | %%A) + 0x34"
